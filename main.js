...
function getInfo(name, login, location, avatar, url) {
	...
	if(location == null) {
		infoLocation.textContent = 'Unknown';
	} else {
...
function getRepos(name, url, description, language, all_languages) {
	let githubRepos = document.getElementById('github-repos'),
		repo = document.createElement('div'),
		repoLink = document.createElement('a'),
		repoDesc = document.createElement('p'),
		repoCircle = document.createElement('span'),
		repoLang = document.createElement('p');
	repo.className = 'repo';
	repoLink.href = url;
	repoLink.textContent = name;
	repo.appendChild(repoLink);
	repoDesc.textContent = description;
	repoDesc.className = 'repos-desc';
	repo.appendChild(repoDesc);
	repoCircle.className = 'circle';
	repoCircle.style.backgroundColor = colorsJson[language];
	repo.appendChild(repoCircle);
	repoLang.textContent = language;
	repoLang.className = 'repos-lang';
	repo.appendChild(repoLang);
	githubRepos.appendChild(repo);
}
			...
			.then(resp => {
				if(resp.message != "Not Found") {
					...
					for (let i = 0; i < resp.length; i++) {
						getRepos(resp[i].name, resp[i].html_url, resp[i].description, resp[i].language, resp[i].languages_url);
					}
				}
			});
	}
